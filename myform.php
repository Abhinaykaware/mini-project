...
input::-webkit-inner-spin-button {
 ...
 <body  onload=display_c();>
  ...
  if(empty($_POST["purpose"]))
	...
	$p = $_POST["purpose"];
//date_default_timezone_set("Asia/indian");
//$timein = date("H:i:s");
...
<div style="margin-left:110px;padding-bottom:12px">
  ...
  <input autocomplete="off" class="form-control" type= "text" name ="name" placeholder= "" required id = "name"
...
<label for="cno"> Contact No. :</label> <span id = "span" style = "padding-bottom: 5px;float:right;"></span>
 <input autocomplete="off" class="form-control" type="number" id = "ContactInfo" onkeyup = "Ccheck('ContactInfo')" 
	onblur = "isEmpty('ContactInfo')" onfocus = "onfo('ContactInfo')" name="cno" placeholder="" 
...
<input autocomplete="off" class="form-control" type="text" name="purpose" placeholder="" required id = "Purpose" 
  ...
  <div class="form-group">
   <label for = "meetingTo">Meeting to :</label>
    <input autocomplete="off" class="form-control" type="text" required name = "MeetingTo" id = "meetingTo" 
	   placeholder=""       oninvalid="this.setCustomValidity(this.willValidate?'':'Whom do you want to meet ?')" maxlength="30"  onblur="isEmpty('meetingTo')"
	   data-toggle="popover" title="Popover Header" data-content="Some content inside the popover" data-trigger = "onfocus"/>
   </div>
  </div>
